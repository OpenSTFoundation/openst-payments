...
library MerklePatriciaProof {
    ...
    function verify(bytes32 value, bytes encodedPath, bytes rlpParentNodes, bytes32 root) internal returns (bool) {
        ...
        if(path.length == 0) {
            emit TestEvent(currentNode,0,0);
            return false;
        }

        for (uint i=0; i<parentNodes.length; i++) {
            if(pathPtr > path.length) {
                emit TestEvent(currentNode,i,1);
                return false;
            }

            currentNode = RLP.toBytes(parentNodes[i]);
            if(nodeKey != keccak256(currentNode)) {
                emit TestEvent(currentNode,i,2);
                return false;
            }
            ...
            if(currentNodeList.length == 17) {
                if(pathPtr == path.length) {
                    if(keccak256(RLP.toBytes(currentNodeList[16])) == value) {
                        emit TestEvent(currentNode,i,3);
                        return true;
                    } else {
                        emit TestEvent(currentNode,i,4);
                        return false;
                    }
                }
                ...
                if(nextPathNibble > 16) {
                    emit TestEvent(currentNode,i,5);
                    return false;
                }
