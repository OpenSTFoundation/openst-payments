...
ContractInteractHelper.prototype = {
  ...
  getTransactionReceiptFromTrasactionHash: function(web3Provider, transactionHash) {
    return new Promise(function(onResolve, onReject) {
      ...
      const getReceipt = async function() {
        if (maxAttempts > 0) {
  ...
  performSend: function(params, returnType) {
    ...
    const notifyTransactionInitiated = function(transactionHash) {
      ...
      openStNotification.publishEvent.perform(notificationData).catch(function(err) {
        logger.error(
          'Message of kind transaction_initiated was not published. Notification data: ',
          notificationData,
          ' Error: ',
          err
        );
    ...
    const notifyTransactionMined = function(receipt) {
      // Publish event
      notificationData.message.kind = 'transaction_mined';
      notificationData.message.payload.receipt = receipt;
      openStNotification.publishEvent.perform(notificationData).catch(function(err) {
        logger.error(
          'Message of kind transaction_mined was not published. Notification data: ',
          notificationData,
          ' Error: ',
          err
        );
    ...
    const notifyTransactionError = function(errorReason) {
      ...
      openStNotification.publishEvent.perform(notificationData).catch(function(err) {
        logger.error(
          'Message of kind transaction_error was not published. Notification data: ',
          notificationData,
          ' Error: ',
          err
        );
    ...
    const onReceiptSuccess = async function(receipt) {
      ...
      notifyTransactionMined(receipt);
    ...
    const onCatch = function(reason) {
      if (reason && reason.message && reason.message.includes('not mined within')) {
        ...
        let errorCode = 'l_ci_h_pse_2';
        // Below error codes are used by openst payments caller. Please don't change.
        if (reason && reason.message && reason.message.includes('insufficient funds for gas * price + value')) {
          errorCode = 'err_gas_too_low';
          reason.message = 'insufficient gas';
        } else if (
          reason &&
          reason.message &&
          (reason.message.includes('Invalid JSON RPC response') || reason.message.includes('connection not open'))
        ) {
          errorCode = 'err_chain_node_down';
          reason.message = 'chain node is down';
        } else if (reason && reason.message && reason.message.includes('is out on sync with other')) {
          errorCode = 'err_chain_node_sync_error';
        } else if (
          reason &&
          reason.message &&
          (reason.message.includes('nonce too low') ||
            reason.message.includes('Transaction nonce is too low. Try incrementing the nonce'))
        ) {
          errorCode = 'err_nonce_too_low';
          reason.message = 'nonce too low';
        } else if (
          reason &&
          reason.message &&
          (reason.message.includes('replacement transaction underpriced') ||
            reason.message.includes('known transaction'))
        ) {
          errorCode = 'err_replacement_tx_under_priced';
          reason.message = 'trying to send tx with an already consumed nonce';
        }
...
*/
          ...
          .then(function() {
            ...
            if (processReceipt) {
              web3Provider.eth
                ...
                .once('rawTransactionDetails', function(rawTx) {
                  logger.debug(`\n======rawTransactioDetails======: ${rawTx}`);
                  rawTxDetails = rawTx;
                })
                .on('transactionHash', function(transactionHash) {
                  logger.debug(`\n=======transactionHash received========: ${transactionHash} \n`);
                  const onTransactionHashResponse = onTransactionHash(transactionHash);
                  onTransactionHashResponse.data.rawTxDetails = rawTxDetails;
                ...
                .catch(function(reason) {
                  ...
                  if (isValueReturned == false) {
                    isValueReturned = true;
                    onCatch(reason).then(function(customResponse) {
                      customResponse.data.rawTxDetails = rawTxDetails;
                      return onResolve(customResponse);
                    });
                  }
                });
            } else {
  ...
  validatePayParams: async function(
    ...
    if (!basicHelper.isVarNull(txNonce) && !basicHelper.isVarInteger(txNonce)) {
      let errorParams = {
        internal_error_identifier: 'l_ci_h_vpp_10',
        api_error_identifier: 'invalid_api_params',
        error_config: errorConfig,
        params_error_identifiers: ['nonce_invalid'],
        debug_options: {}
      };
      return Promise.reject(responseHelper.paramValidationError(errorParams));
    }
   ...
   *
   * @params {object} postPayParams -
   * @param {string} postPayParams.beneficiaryAddress - beneficiary address
   * @param {string} postPayParams.commissionBeneficiaryAddress - commission beneficiary address
   * @param {string} postPayParams.spender - spender address
   * @param {string} postPayParams.brandedTokenAddress - branded token address
   * @param {string} postPayParams.contractAddress - contractAddress address
   * @param {string} postPayParams.airdropBudgetHolder - airdrop budget holder address
   * @param {number} postPayParams.totalAmount - total amount that was debited from spender account
   * @param {number} postPayParams.airdropAmountToUse - airdrop amount that was used in the transaction
   *
   * @return {Promise}
   *
   */
